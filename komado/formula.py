...
class _Parser:
    ...
    def _comparison(self) -> Node:
        node = self._expr()
        op = self._match_op(*_CMP_OPS)
        if op is not None:
            node = ("cmp", op, node, self._expr())
        return node
...
def _fn_power(values: list[float]) -> float:
    if len(values) != 2:
        raise FormulaError("#ERROR!", "POWER の引数は2個(底, 指数)")
    result = values[0] ** values[1]
    if isinstance(result, complex):
        raise FormulaError("#ERROR!", "負の数を非整数乗できない")
    return float(result)


def _fn_mod(values: list[float]) -> float:
    if len(values) != 2:
        raise FormulaError("#ERROR!", "MOD の引数は2個(被除数, 除数)")
    if values[1] == 0:
        raise FormulaError("#DIV/0!", "0で割った余り")
    return values[0] % values[1]


# 範囲(A1:A5)を畳んで1つの数にする関数。範囲・複数引数を受け取る。
...
# 引数を要素ごとに扱う関数。第1引数(以降)をそのまま受け取る。
_SCALARS: dict[str, Callable[[list[float]], float]] = {
    ...
    "POWER": _fn_power,
    "MOD": _fn_mod,
    "FLOOR": lambda xs: float(math.floor(xs[0])),
    "CEIL": lambda xs: float(math.ceil(xs[0])),
    "INT": lambda xs: float(math.trunc(xs[0])),
...
def _apply(name: str, fn: Callable[[list[float]], float], values: list[float]) -> float:
    """関数を適用し、定義域外(負数の平方根など)の失敗を式エラーに変える。

    関数が自前で投げる FormulaError はそのまま通し、Python標準の
    ValueError / 算術エラーだけを #ERROR! に丸める。
    """
    try:
        return fn(values)
    except (ValueError, ArithmeticError) as exc:
        raise FormulaError("#ERROR!", f"{name} の計算に失敗: {exc}") from exc
...
class Engine:
    ...
    def _eval_node(self, node: Node) -> float:
        ...
        if kind == "cmp":
            return self._eval_cmp(node[1], node[2], node[3])
        if kind == "call":
    ...
    def _eval_cmp(self, op: str, left: Node, right: Node) -> float:
        """比較は真を 1、偽を 0 として返す。IF と組み合わせて使う。"""
        ...
        return 1.0 if outcomes[op] else 0.0
    ...
    def _eval_call(self, name: str, args: list[Node]) -> float:
        if name == "IF":
            if len(args) != 3:
                raise FormulaError("#ERROR!", "IF の引数は3個(条件, 真のとき, 偽のとき)")
            chosen = args[1] if self._eval_node(args[0]) != 0 else args[2]
            return self._eval_node(chosen)
        if name in _AGGREGATES:
            ...
            return _apply(name, _AGGREGATES[name], numbers)
        if name in _SCALARS:
            if not args:
